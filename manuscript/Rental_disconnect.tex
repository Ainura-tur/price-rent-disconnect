...
\citet{sharpe2019reevaluating} attributes a substantial share of US rental
...
private Gross Fixed Capital Formation (GFCF; ABS State Final Demand);
labour market conditions by employment (ABS 6202.0) and wages, measured by
the Wage Price Index (WPI; ABS 6345.0);
...
Post-2017 corr.\ decline$^{a}$ & Small & Large & None
& None & None \\
Geographic constraint$^{b}$ & Severe & Moderate & Low
& Low & Moderate \\
\bottomrule
\end{tabular*}
...
$^{a}$Large = post-2017 rolling correlation decline $> 0.30$;
Small = between 0 and $0.30$; None = no decline. Reported descriptively,
not as a financialization-intensity index. $^{b}$Following
...
serve as proxies for city-level measures for GFCF and
...
claim it would be concentrated in particular cities; our finding that the
post-2017 strengthening is broad-based across the capitals rather than
concentrated in any one of them gives no reason to expect a uniform
national measure of this kind to bite unevenly across capitals.
...
The data and code required to reproduce the results in this paper are openly
available in the replication repository at
\url{https://github.com/Ainura-tur/price-rent-disconnect}. The repository
contains the full estimation pipeline and the raw input series obtained from the
Australian Bureau of Statistics (under CC BY 4.0), the Reserve Bank of Australia,
and the Bank for International Settlements, each redistributed under its source's
terms with attribution as documented therein.
